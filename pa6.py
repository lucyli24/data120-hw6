...
class DTree:
    def __init__(self, variable, threshold, lessequal, greater, outcome):
        if (variable is None and threshold is None and lessequal \
           ...
           (variable is not None and threshold is not None and lessequal is not None \
    ...
    def find_outcome(self, tup):
        '''
        Takes in a tuple with observations and navigates through the tree to provide
    ...
    def no_repeats(self):
        '''
        Analyzes the tree and returns True if and only if there are not “repeats”, False otherwise
        '''
